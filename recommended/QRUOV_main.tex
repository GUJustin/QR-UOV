...
\section{From the expanded key to universal forgery}\label{sec:model}

The direct attack works with the official base-field map, while equivalent-key
recovery works first with its quotient-ring expansion.  Equivalent-key
recovery is complete only after the recovered structure gives a base-field
decomposition that can invert every verification target.  This section defines
the expansion model used for average-case claims and gives the two public
checks that turn a recovered graph into a universal forgery.

\subsection{Coordinates, parameters, and expansion model}

The Round-2 parameter sets use $q=127$ and quotient degree $\ell=3$.  The
official map has $v=\ell V$ vinegar variables, $m=\ell O$ oil variables, and
$m$ outputs over $\F_q$.  Its quotient-ring expansion has the same $m$ outputs
in $N=V+O$ variables over
\[
...
\end{tabular}
\end{center}
We write $D=2^V$ for the B\'ezout degree of a smooth square core and
$L_s=\F_{Q^s}$ for the solver's working field.  All concrete theorems below
concern these three sets, for which $m<q$.

Write the expanded public tuple as
\[
 P=(p_1,\ldots,p_m),\qquad
 p_i(x)=x^{\mathsf T}P_ix,
 \qquad
 P_i=\begin{pmatrix}A_i&E_i\\E_i^{\trans}&C_i\end{pmatrix}.
\]
In normalized public coordinates, the hidden oil space is the graph
\begin{equation}\label{eq:graph}
 \cO=\left\{\binom{-Gc}{c}:c\in K^O\right\}.
\end{equation}
It is totally isotropic for every public form,
...
\end{equation}
The corresponding secret central form is
\begin{equation}\label{eq:central}
 F_i=\begin{pmatrix}A_i&B_i\\B_i^{\trans}&0\end{pmatrix},
 \qquad
 F_i(v,o)=v^{\trans}A_iv+2v^{\trans}B_io,
 \qquad B_i=E_i-A_iG.
\end{equation}

\begin{definition}[Public-random expanded key]\label{def:ideal}
...
The equivalent-key analyses use Definition~\ref{def:ideal}; their bad-key
events depend on the uniform central pairs and on fresh attack coins, not on
independence between $G$ and $A$.  The direct reduction is key by key, but its
average-case residual-root theorem conditions on an off-oil point and then
requires $G$ to be independent of the central coefficient pairs.  It therefore
uses Definition~\ref{def:direct-ideal}.  The ideal-XOF and ideal-cipher
transfers, and the gap for fixed SHAKE or AES, are stated in
\cref{app:seeded-transfer}.  These models govern only the probability of
finding a certificate.  Every returned key and signature is checked against
the unchanged public map.
...
invertible pull-back therefore gives matrices
...
For an expanded base-field decomposition
$\widehat P_i=\widehat S^{\mathsf T}\widehat F_i\widehat S$, write the central
variables as $(a,o)\in\F_q^v\oplus\F_q^m$.  Fixing $a$ makes the central map
linear in $o$ because every oil-oil block is zero.  Define this linear map by
...
every target $y\in\F_q^m$, compute
\[
...
query.  Given a message $M$, choose any allowed salt $r$, compute
\[
 \mu=\operatorname{SHAKE256}(\mathsf{seed}_{\rm pk}\mathbin\|\operatorname{BytesToBits}(M),512),
 \qquad y=\operatorname{Hash}(\mu,r),
\]
invert $y$ with Proposition~\ref{prop:signing-witness}, and output $(r,x)$.
\end{corollary}

\begin{proof}
The verifier accepts exactly when
$\widehat P(x)=\operatorname{Hash}(\mu,r)$~\cite[Secs.~4.2--4.3]{QRUOVSpec}.
The certificate inverts every target under this unchanged map.  It neither
recovers nor uses the secret seed.
\end{proof}

\subsection{Finding an invertible oil system}
...
\begin{proposition}[Basis-first signing search]\label{prop:signing-search}
...
\]
If $\Delta$ is nonzero, a uniform scalar-block vinegar assignment succeeds
...
certificate even for a candidate that is not planted.
\end{proposition}
...
The direct attack works with the official base-field verification map
\[
 \widehat P:\F_q^n\longrightarrow\F_q^m,
 \qquad n=v+m,
 \qquad q=127.
\]
Given a nonzero target $y$, the attack constructs an $(m-3)$-dimensional
public subspace on which three zero-target equations vanish identically.  On
this subspace, the remaining zero-target equations form a square projective
system of dimension $m-4$.  The final nonzero-target equation only recovers
the affine scale.  The resulting solver cores are
\[
 50,\qquad74,\qquad101
\]
...
equations modulo scaling.  This removes one more variable, giving cores of
dimension $m-4$.  We also give a finite Chevalley--Warning construction, a
fixed-key seed bound, an explicit nonsingular-root probability for the
degree-three pull-back, and a finite-characteristic solver cost.
\end{remark}
...
arbitrary correlation between $G$ and $A$.  In the ideal-XOF and ideal-cipher models,
...
test whether $w$ is off oil.  The random seed subspace makes this test
unnecessary; the next proposition bounds the failure probability for every
fixed oil space.
...
\begin{proof}
At each step, intersect the polar-orthogonal spaces of the current isotropic
subspace.  Condition \eqref{eq:cw-feasibility} leaves at least $2s+1$
...
is a nonzero square.  Euler's identity identifies affine nonsingularity with
projective nonsingularity after removing the radial direction.  Counting the
...
gates across the three profiles.  The complete ledger appears in
\cref{app:direct-proofs}.

\subsection{Why a usable projective root exists}

The degree-three pull-back does not produce every UOV quadric over $\F_q$.
The root count therefore uses the exact local randomness that the pull-back
does provide.  Let $E=K^{V+O}$, viewed over $\F_q$, and let
$\widehat{\cO}\subset E$ be the hidden base-field oil space.  In central
coordinates, each public-random pulled-back form has the form
...
\begin{lemma}[Evaluation and first-derivative randomness]\label{lem:pullback-one-jet}
...
surjectivity and degree-three descent argument appears in
\cref{app:direct-proofs}.
\end{proof}
...
\begin{proof}
By Corollary~\ref{cor:projective-target-slicing}, every affine nonsingular root of
\eqref{eq:direct-residual} maps to a nonsingular projective root with the
...
invocation reaches an off-oil residual with a represented nonsingular
$\F_q$-rational projective root whose last quadratic value is a nonzero square
...
pull-back forms.  Corollary~\ref{cor:projective-root-probability} supplies a usable
...
Combining the direct reduction with directional equivalent-key recovery gives
...
core of dimension $V$;
\item if $V\ge m-\ell$, the direct reduction with $s=\ell$, followed by
...
Corollary~\ref{cor:projective-target-slicing} removes its radial coordinate, leaving
...
The direct construction and the complete local-separator pipeline have both
...
Fixing a public direction produces a quadratic system in only $V$ variables.
The hidden graph gives this system a planted root.  If the derivative at that
root has full rank, public linear algebra recovers every column of the graph.
The attack then verifies the graph identities, the pull-back, the public-map
factorization, and a signing witness.  A separate projective route recovers the
same oil space directly from the common derivative kernel.

\subsection{A public system with a planted root}
...
\begin{equation}\label{eq:direction-system}
 ...
 \qquad x\in K^V.
\end{equation}
The equations $q_{1,c}=\cdots=q_{m,c}=0$ are public.  Define their public
derivative matrix by
...
\cref{prop:verified-key}.
\end{proof}

\begin{remark}[A compact attack certificate]\label{rem:graph-certificate}
A successful branch is certified by $(c,x,G)$ and one full-rank minor of
$\mathcal D_c(x)$.  A verifier checks the directional equations, the graph
...
\end{equation}
In the public-random model, $\mathcal B(c)$ is a uniform $m\times V$ matrix for
...
\subsection{Recovering the oil space by linear algebra}

The faster projective route from \cref{sec:projective} instead finds a public
common zero $x$ on the oil space.  Form
\[
...
At the planted oil point, smoothness of the restricted core gives derivative
rank $V$.  The lemma therefore recovers the full oil space.  Row reduction,
descent to $K$, and the checks from \cref{prop:verified-key,prop:signing-search}
...
\section{The solver needs only a full-rank Jacobian}\label{sec:local-geometry}
...
\begin{equation}\label{eq:local-jacobian-polynomial}
 h(x):=\det J_f(x).
\end{equation}
The root solver used in the next section asks that the equations form a
regular sequence and that every prefix ideal be radical, but only after
localization.  Taking $h$ as the localization polynomial removes every point
...
then uses the complete-intersection criterion in the regular ring $S_h$.
...
\[
 \{f_1=\cdots=f_V=0,\ h\ne0\}.
\]
The input tuple $(f_1,\ldots,f_V,h)$ satisfies hypotheses (H1)--(H2) of the locally closed Kronecker algorithm of Gim\'enez et al.~\cite{Gimenez2026}.  These conclusions are independent of every root and irreducible component contained in $h=0$.
\end{corollary}

\begin{proof}
Equation \eqref{eq:h-at-planted} puts the planted root in the open set.  Lemma~\ref{lem:jacobian-localization} gives exactly the two algebraic conditions required after localization.
\end{proof}

Thus this recovery route needs local smoothness at the planted root, not global
smoothness of the projective core.  Singular roots and components contained in
$h=0$ do not affect the conclusion.
