...
\section{Reconstructing the time and memory frontier}\label{app:memory-frontier}

The main paper compares attack estimates in both time and storage.  This
section gives the formulas behind that comparison.  The companion script
\texttt{QRUOV\_memory\_audit.py} evaluates every binomial coefficient as an
integer and checks that the resulting time estimates reproduce the published
PXL, reconciliation, rectangular MinRank, and wedge rows to their stated
precision.

\subsection{WXL and matrix-free storage}

Consider $m$ quadratic equations in $n$ variables over a field of size $q$.
After guessing $k$ variables, the semi-regular WXL estimate defines
$d_{\rm reg}$ as the first non-positive coefficient of
\[
 \frac{(1-z^2)^m}{(1-z)^{n-k+1}}.
\]
The square matrix used in the estimate has dimension
\[
 N=\binom{d_{\rm reg}+n-k}{d_{\rm reg}},
 \qquad
 \rho=\binom{n-k+2}{2}
\]
nonzero entries per row.  The published gate estimate is
\[
 q^k\,3\rho N^2\bigl(2(\log_2q)^2+\log_2q\bigr).
\]
A stored sparse matrix needs about
$N\rho(\log_2N+\log_2q)$ bits.  If the entries are regenerated, Wiedemann
linear algebra needs only a constant number of length-$N$ vectors.  We count
three packed vectors in the prior-work comparison.  This favors the published
attacks because it omits matrix-generation overhead and any implementation
workspace beyond those vectors.

For the updated Hashimoto rows, the generalized partition analysis gives
dominant residual systems with $(n,m)=(49,51),(72,75),(99,102)$ before WXL's
own inner guessing step~\cite{GeneralizedPartition}.  Optimizing the displayed
formula gives
\[
 \log_2N=53.452,\quad76.525,\quad109.277.
\]
The reconciliation attack uses the same WXL solver over the QR-UOV quotient
field $\F_{127^3}$.  Its residual system dimensions are
$(n,m)=(52,54),(76,78),(102,105)$, and the same calculation gives
\[
 \log_2N=71.302,\quad104.121,\quad133.438.
\]

Our exact reduction gives square systems with $n=m=50,74,101$.  Optimizing
WXL for time gives costs
\[
 159.223,\quad218.144,\quad285.668
\]
and matrix dimensions with base-two logarithms
\[
 52.953,\quad78.340,\quad104.670.
\]
These are not the minimum-memory choices.  Fixing the hybrid parameters to
$k=27,45,70$ leaves respectively $23,29,31$ variables.  In all three cases
$d_{\rm reg}=5$, and
\[
 N=98{,}280,\quad278{,}256,\quad376{,}992.
\]
Regenerating the matrices and storing three vectors plus the reduced equations
in 64-bit words takes about $2.4$, $6.9$, and $9.5$ MiB.  The corresponding
gate estimates are
\[
 238.387,\quad367.819,\quad543.595.
\]
These are $19.613$, $31.181$, and $17.405$ bits below the current classical
Just Guess estimates~\cite{JustGuess,GeneralizedPartition}.  This comparison
does not make the attacks practical, since all three running times remain far
beyond computation.  It shows that the new reduction also improves the
low-memory end of the modeled frontier.

\subsection{PXL storage}

The PXL estimate in the QR-UOV specification uses
\[
 \alpha=\sum_{d=0}^{d_{\rm reg}}
 \max\left\{[z^d](1-z)^{m-(n-k)}(1+z)^m,0\right\}
\]
and includes an $\alpha^\omega$ elimination term~\cite{QRUOVSpec}.  A usual
dense realization of this step stores an $\alpha\times\alpha$ field matrix.  We
therefore report $\alpha^2\lceil\log_2q\rceil$ bits.  This is an implementation
model rather than a lower bound.  A lower-space dense elimination schedule can
trade additional time for storage.

After the Thomae--Wolf reduction used in the specification, PXL solves square
systems of sizes $52,76,103$.  The optimized values satisfy
\[
 \log_2\alpha=36.220,\quad52.779,\quad71.908,
\]
which reproduce the published rounded gate estimates $150,211,279$.  Applying
the same formula to our square cores of sizes $50,74,101$ gives
\[
 \begin{array}{c|ccc}
  &\text{Level I}&\text{Level III}&\text{Level V}\\ \hline
 k&7&10&13\\
 d_{\rm reg}&16&22&29\\
 \log_2\alpha&34.369&50.968&70.168\\
 \log_2\text{gates}&143.643&204.845&272.975\\
 \log_2\text{bytes}&68.546&101.742&140.144
 \end{array}
\]
The structural reduction therefore saves $6.795,5.728,5.949$ time bits and
$3.700,3.623,3.480$ dense-storage bits relative to the specification's PXL
instances.  The resulting memories remain far beyond physical storage.

\subsection{Support-minors, wedge, and depth-first search}

The extended rectangular MinRank paper supplies the optimal ranks, expected
solution dimensions, and Macaulay degrees for the current QR-UOV parameters
over $\F_{127^3}$~\cite{Suzuki}.  Its support-minors matrix has
\[
 N=\binom{n-D+1+b-1}{b}\binom{m}{r},
 \qquad
 \rho=(r+1)(n-D+1).
\]
Substitution of the published tuples gives
$\log_2N=68.597,97.298,126.153$.  Counting three regenerated-matrix vectors
then gives $\log_2$ storage in bytes of $71.575,100.275,129.130$.

For the odd-characteristic wedge attack, Ran's optimistic pruned matrix has
\[
 N=\frac{1}{V+1}
 \binom{V+O'}{V}\binom{V+O'+1}{V},
 \qquad \rho=(V+1)^2
\]
over $\F_{127^3}$~\cite{RanWedges}.  The published choices
$O'=13,17,22$ give $\log_2N=84.310,117.280,156.166$.  Its explicitly stated
regeneration option therefore needs vector storage with base-two byte
logarithms $87.288,120.258,159.143$ under the same three-vector convention.

Just Guess has a different profile.  Its search is depth first and does not
construct a Macaulay matrix.  Storing the $m$ quadratic forms and the two
change-of-basis matrices takes
\[
 m\binom{n+1}{2}+n^2+m^2
\]
field elements.  Using one 64-bit word per element gives about $9.8$, $30$,
and $73$ MiB before language and data-structure overhead.  This is why the
published Just Guess route occupies the low-memory end of the previous
frontier even though its classical running times are much larger.
...
\bibitem{RanWedges}
L.~Ran.
\newblock Wedges, oil, and vinegar: An analysis of UOV in the exterior algebra.
\newblock Cryptology ePrint Archive, Paper 2025/1143, 2025.
