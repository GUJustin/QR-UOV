...
\begin{abstract}
We give faster attacks on the \QRUOV{} signature scheme, a third-round candidate in NIST's Additional Digital Signature process.  For the Round~2 Levels~I, III, and V parameters, our principal equivalent-key attack has uniform $T\log T$ cost indicators about
\[
2^{124.5},\qquad 2^{177.8},\qquad 2^{235.4},
\]
against targets $2^{143}$, $2^{207}$, and $2^{272}$ and the submitters' reported 150, 211, and 277 bits.

The attack restricts the public extension-field quadrics to a random projective $V$-plane and solves a random square recombination.  A smooth core contains a unique oil point, whose ambient derivative kernel is the entire hidden oil space.  We replace the previous generic-key caveat by explicit ideal-field bad-key bounds below $2^{-364.5}$, $2^{-457.9}$, and $2^{-1038.0}$.  All estimates use the specification's asymptotic cost model; no full-parameter attack was executed.
\end{abstract}
...
\]
Second, the verifier is a base-field quadratic map with shapes $(n,m)=(210,54),(306,78),(411,105)$. Our equivalent-key attack uses the first description; the pseudo-oil direct-forgery path uses the second.

\paragraph{Concurrent structural cryptanalysis.}
This paper is one of three concurrent, self-contained, technically independent studies by the same authors in a common program of structural cryptanalysis of UOV-derived signatures.  The other works analyze \MAYO{}/UOV using polar-flag quotients and Hodge-contraction acceleration, and odd-characteristic \SNOVA{} using a symmetric-square quotient of the ordered power-pair feature space~\cite{ConcurrentMAYO,ConcurrentSNOVA}.  None of the three papers is needed for the correctness of either of the others.  They share a broader lesson: the public algebra used to compress or structure a UOV-derived construction must be incorporated explicitly into its security analysis rather than erased by modeling the resulting system as generic MQ.

\subsection{Main attack}

Let $P_1,\ldots,P_m$ be the public symmetric matrices over $K$, acting on $K^{V+O}$, and let $\cO$ be their hidden common totally isotropic $O$-space. The attacker samples
\[
M\in L^{(V+O)\times(V+1)},
\]
where $L/K$ is the extension already constructed by the fast finite-field Kronecker solver, and rejects unless $M$ has full column rank.  Its image is then a projective $V$-plane. Its underlying vector space has dimension $V+1$, while $\cO_L$ has dimension $O$ in an ambient space of dimension $V+O$; hence the two always intersect.

Restrict the public quadrics to this plane and take $V$ random public linear combinations. When the square core is a smooth complete intersection, it has exactly $2^V$ distinct regular roots. Its intersection with the oil space is then one $L$-rational projective point: a larger oil intersection would be positive-dimensional. At that point, total isotropy and smoothness imply that the ambient derivative kernel is exactly $\cO_L$. The attacker row-reduces the kernel, verifies Frobenius descent and every public oil-space identity, and applies the specification's public inverse pull-back.

The previous fixed-slice formulation proved fast regularization only outside a proper algebraic locus whose density over $K$ was unknown. That caveat cannot be removed for a fixed slice: its exceptional locus has low-codimension determinantal components. Randomizing the projective slice changes the exceptional set. We prove nonvanishing of the joint slice/output discriminant outside three explicit loci: no regular oil direction, an off-oil common zero of derivative rank below $V$, or linear dependence of the central forms. The dominant off-oil incidence has projected codimension
\[
e(O+1)+1,\qquad e=m-V,
\]
namely $39,55,109$, and an effective degree/point-count argument yields the stated key-density bounds.
...
For Levels~I, III, and V, the fast projective-slice attack has expected direct/uniform indicators
\[
117.59/124.47,\qquad170.39/177.80,\qquad227.59/235.42.
\]
Its ideal-field key-bad probability is below $2^{-364.56},2^{-457.92},2^{-1038.06}$. Outside that set, one projective slice/output trial over the existing solver fields fails the full-core smoothness test with probability at most $2^{-145.65},2^{-183.46},2^{-219.51}$.
\end{theorem}

\subsection{Why the fallback is not deleted outright}

The new line strictly improves the arbitrary-core method's cost, but not its success set. The arbitrary-core method needs only one nonsingular planted coordinate slice; its exact all-coordinate-slices-deficient probabilities are below
\[
2^{-1132.17},\qquad2^{-1635.35},\qquad2^{-2935.25}.
\]
It may therefore succeed on keys belonging to the fast joint exceptional set. We remove it from the abstract's numerical comparison and the primary security ledger, but retain its complete finite-characteristic specialization in \cref{app:fallback}. 
\begin{table}[t]
\centering
\caption{Headline attacks. Entries are direct indicators, with uniform $T\log T$ indicators in parentheses.}
\label{tab:headline}
...
& Ideal-field theorem; fixed-budget concrete success differs by $\Adv+\varepsilon_{\rm RS}$
...
\paragraph{C1. Random projective slicing.} Every projective $V$-plane contains an oil point; smoothness makes it unique, and its derivative kernel is the full oil space.

\paragraph{C2. Finite-field density.} We isolate three sufficient key-level obstructions, prove joint discriminant nonvanishing outside them, and bound the dominant off-oil incidence by explicit codimension and cumulative degree.

\paragraph{C3. Full-core regularization.} A smooth $2^V$-point core forces the regular-sequence and absolutely radical prefix hypotheses through the a posteriori criterion of van der Hoeven and Lecerf~\cite{vdHL}; the previous product of all prefix discriminants is unnecessary.

\paragraph{C4. Public certification and structure.} We give derivative-kernel recovery, Frobenius descent, one-column graph completion, a low-degree completion observable, and a global $2^V$ multihomogeneous path-count floor.

\paragraph{C5. Secondary direct forgery.} A pre-slice pseudo-oil construction reduces the full base-field map to a final $(m-3)$-variable square system; post-slice enrichment is provably useless in the adopted framework.

\paragraph{C6. Robustness.} The slower arbitrary-core solver is retained in an appendix because its coverage guarantee is incomparable with the main theorem's and has a smaller explicit bad-key probability, not because it is competitive with the main line.

The algebraic reductions and probability bounds are exact where stated.  Key-density calculations use the ideal-field experiment; fixed-budget verified-success probabilities transfer computationally to SHAKE/AES expansion.  Fast-solver and fallback costs are asymptotic bounds with enormous hidden constants and memory and are not implementation forecasts.  Pseudo-oil existence, extraction, and branch behavior use the cited average-case heuristic.

\section{The public extension-field instance}\label{sec:model}
...
Whenever \textsf{Expandpk} or \textsf{Expandsk} requests an element of $\F_q$, sample it independently and uniformly from $\F_q$, then run all remaining key-generation steps unchanged.
\end{definition}

In extension-field block notation, the public top-left and top-right blocks and the secret graph are independent and uniform in this experiment. The central matrices $A_i,B_i$ below are consequently independent and uniform.
...
\subsection{Coordinate batch retained for the fallback}

For $1\le j\le O$, fixing the final public coordinates to $e_j$ leaves a $V$-variable affine system with planted point $(-Ge_j,e_j)$. In central coordinates and after translation it is
\[
...
Fix the planted oil space $\cO_0$.  Let $W\ne\cO_0$ be another $O$-space and put
\[
...
% The next modular section is the complete new geometric and finite-field proof.
% BEGIN random_projective_attack.tex
...
Combining Theorem~\ref{thm:joint-nonzero} with
...
For every key outside the bad set of Theorem~\ref{thm:key-density}, if $M$ and $R$ are uniform over a finite extension $L/K$, then
...
% END random_projective_attack.tex
...
\]
It uses the enriched pseudo-oil framework of May--Ostuzzi--Ressler and Ostuzzi~\cite{JustGuess,Ostuzzi}.  We retain this path because it constrains a different parameter surface: its terminal square-system dimension is governed mainly by $m$, whereas the fast equivalent-key attack is governed mainly by $V=v/3$.
...
r=m-B-k.
\]
The two load-bearing feasibility inequalities in the adopted average-case model are
\begin{align}
...
k=0.
\]
Then $B=2$, $L_{\rm po}=3$, and $r=m-2$.  For the three recommended shapes, the left side of \eqref{eq:po-one} is respectively $6,6,3$, while the right side is $3$; \eqref{eq:po-two} has slack $52,76,100$.  Thus the adopted heuristic predicts a valid decomposition at every level.  The centrifugation constructs
\[
...
Conditioned on a successful centrifugation with the parameters above, direct forgery reduces exactly to:
\begin{enumerate}
...
The primary pseudo-oil line uses the field-uniform arbitrary-core solver from \cref{app:fallback} on the final $(m-3)$-variable system and therefore inherits the preceding nonsingular-terminal-branch assumption.  For the selected two one-dimensional blocks, the charged centrifugation work is $(m-2)\Solve(3,3)+\Solve(1,1)$ plus polynomial-time linear algebra; the constant-size branch tree is also lower order than the terminal solve in the adopted model.
...
The solver output is astronomically large.  One degree-$2^V$ quotient vector over the relevant auxiliary field occupies approximately $0.102$ EiB, $2.23\times10^6$ EiB, and $1.84\times10^{14}$ EiB at Levels~I, III, and V; materializing all coordinate polynomials is larger by another factor of roughly $V$.  No implementation in this work approaches the full-parameter time or memory.  The result is a parameter shortfall in the specification's asymptotic attack model, not a practical key recovery today.

\section{Related work}\label{sec:related}

\paragraph{Concurrent, technically independent work.}
Concurrent papers by the present authors also exploit public algebra in compact UOV-derived designs~\cite{ConcurrentMAYO,ConcurrentSNOVA}.  The \MAYO{}/UOV paper develops a characteristic-two, preimage-preserving quotient of the verification map on polar-isotropic subspaces and, separately, accelerates an exterior-algebra key-recovery product through Hodge contractions.  The \SNOVA{} paper identifies an odd-characteristic symmetric-square quotient of the homogeneous ordered power-pair feature space inside an affine-column forgery attack.  The present key recovery uses neither a polar or symmetric-square quotient nor an exterior Macaulay system: it samples a projective extension-field slice, solves one planted square core, and recovers the complete oil space by an ambient derivative kernel and public graph completion.  Its pseudo-oil forgery likewise uses a separate centrifugation construction and is not a specialization of the \MAYO{} quotient.  Thus the three works share a structural-cryptanalysis viewpoint but have distinct central theorems, attack pipelines, and assumptions.
...
May, Ostuzzi, and Ressler's \emph{Just Guess} and Ostuzzi's enriched pseudo-oil framework give improved algorithms for underdetermined MQ~\cite{JustGuess,Ostuzzi}.  Our direct-forgery contribution is the post-slice feasibility obstruction, the reverse composition in which centrifugation precedes geometric resolution, and the QR-UOV parameter audit.  Jin et al.\ study odd-characteristic UOV families through symmetric algebra but report no stronger QR-UOV figures in that line~\cite{Jin}.  Side-channel and vinegar-reuse issues are orthogonal and are surveyed by Aulbach, Campos, and Kr\"amer~\cite{Aulbach}.
...
\item Run enriched centrifugation with $p=2$, $(b_1,b_2)=(1,1)$, $\ell_{\rm po}=1$, and $k=0$.
...
This is why the fallback is retained but demoted.  Its uniform costs are $14.36,11.97,8.64$ bits above the fast projective line, respectively, while its explicit ideal-field bad-key probability is much smaller.  It does not strictly dominate or become strictly dominated in the joint cost/coverage order.
...
Every corank-at-least-two member is automatically critical and creates a large extraneous component.  Exact finite-field tests confirm the characterization, but it has not yielded a faster attack.
...
The bundle also preserves the preceding v12 artifact for provenance.  Experimental results support only the algebraic identities they test; they are not extrapolated into production runtimes.
...
The key-density and coordinate-batch probabilities use the ideal-field experiment.  Concrete fixed-budget verified-success experiments transfer through \cref{prop:transfer}.  The fast theorem excludes an explicitly bounded ideal-field set.
