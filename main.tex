...
\newcommand{\MAYOPaperTitle}{Cryptanalysis of \MAYO{} and \textsf{UOV} via Polar Flags and Hodge Contractions}
...
For Levels~I, III, and V, the fast projective-slice attack performs about $2^{117.59},2^{170.39},2^{227.59}$ bit operations, equivalently a Boolean circuit of about $2^{124.47},2^{177.80},2^{235.42}$ gates---the unit in which the security targets are stated. Its ideal-field key-bad probability is below $2^{-364.56},2^{-457.92},2^{-1038.06}$. For a key outside that bad set, a single attempt---one random slice $M$ and one random recombination $R$, over the solver fields already in use---fails the smoothness test with probability at most $2^{-145.65},2^{-183.46},2^{-219.51}$.
\end{theorem}
...
May, Ostuzzi, and Ressler's \emph{Just Guess} and Ostuzzi's enriched pseudo-oil framework give improved algorithms for underdetermined MQ~\cite{JustGuess,Ostuzzi}.  Relative to those algorithms, the new parts of our direct-forgery path are the feasibility obstruction that remains after slicing, the reversed order in which subspace extraction precedes geometric resolution, and the parameter audit specialized to QR-UOV.  Jin et al.\ study odd-characteristic UOV families through symmetric algebra but report no stronger QR-UOV figures in that line~\cite{Jin}.  Side-channel and vinegar-reuse issues are orthogonal and are surveyed by Aulbach, Campos, and Kr\"amer~\cite{Aulbach}.
...
Let $t\ge1$, and run $t$ independent public slice-and-output trials.  Amplify the verified internal solver so that the probability that all otherwise-good trials fail internally is at most $\beta_t$.  In the ideal-field experiment, the probability of no verified equivalent key is at most
\[
...
The frozen audit suite covers shapes $(O,m)=(2,4),(3,4),(2,3)$ over $\F_5,\F_7,\F_{11}$.  Across $47$ keys and $510$ slice-and-output trials, $347$ cores were geometrically smooth; every one of those $347$ recovered the exact planted oil space.  This is a controlled identity test, not an extrapolation to production dimensions, but it exercises precisely the projective-intersection and derivative-kernel steps newly introduced here.
...
\section{A pseudo-oil and geometric-resolution direct-forgery path}\label{sec:pseudo-oil}
...
The QR-UOV specification assigns security categories by counting the Boolean gates in a circuit that carries out the attack~\cite{QRUOVSpec}, so all comparisons must ultimately be in that unit.  Our \emph{direct} column is the number of bit operations the attack performs, read off from the cited solver theorems with quasi-linear arithmetic in the relevant finite field.  Our \emph{uniform} column converts that into a gate count: a $T$-step bit algorithm compiles into a Boolean circuit of size $O(T\log T)$~\cite{PippengerFischer}, so the uniform value is the direct value plus a $\log_2$ term.  The uniform column is the one that matches the specification's unit, and it carries the main comparison.
